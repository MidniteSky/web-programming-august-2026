/*
    ...
    WHAT THIS STEP BUILDS (everything - it's the first step):
      ...
      - loadMembers(): ask the server for every member with a GET request,
        using the week's standard shape - fetch + async/await, check
        response.ok, try/catch around the lot
    ...
    WHAT YOU SHOULD SEE when it works:
      ...
      - Active is a green "yes" or grey "no" pill - that's a Bootstrap BADGE
    ...
    NOT WORKING YET (deliberately):
      ...
      - the Add-member modal OPENS (that's Bootstrap's data attributes in
        the HTML, no JS needed) but its Add button RELOADS THE PAGE.
        That's the browser's built-in form submit - the same one we met on
        Day 1. We catch and stop it in Step 3.
...
/* ---- One place for the server address --------------------------------------
   ...
   ever moves (different port, or a hosted API when we deploy), we change
   ONE line, not ten. */
...
/* The line above the table doubles as loading indicator, success note and
   error report. One helper owns it so every message looks the same.
   The classes are Bootstrap text colours: grey normally, red for errors. */
...
/* ==== READ: load all members with GET ======================================
   The week's standard request shape, worth reading slowly because every
   other request today follows it exactly:
...
/* ---- Build one row for one member ------------------------------------------
   ...
   TEXT, never as markup. That habit is non-negotiable all week. */
function buildRow(member) {
    /* Destructure the fields we display (Day 4) - four tidy constants
       instead of member.name, member.email everywhere. */
    ...
    /* Active as a Bootstrap BADGE - a coloured pill instead of Day 4's
       coloured text. Same information, dressed better. */
